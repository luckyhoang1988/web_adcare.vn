...
- **Admin action** "🔄 Lấy bài ngay" (`RssSourceAdmin.fetch_now`) — chạy **nền** (background thread `_fetch_sources_bg`) để tránh timeout gunicorn (mặc định 30s); trả về ngay, làm mới trang sau 1–2 phút để xem `Số bài`/`Lần lấy gần nhất`. Lỗi nền ghi vào `logs/django.log`.
